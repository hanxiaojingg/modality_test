...
##############################################################
bmodetest_cv <- function(y,lower = NULL, upper = NULL,B=1000,lam=NULL,eps1=.5,eps2=.2,eps=0.01,cv=TRUE){
  nraw=length(y)
  q1=quantile(y,.01)
  q2=quantile(y,.99)
  rng=q2-q1
...
ans=bmodetest_cv(y, B=1) ### set B=1 to skip sampling and get the fits
stopImplicitCluster()
### power is about 0.34
